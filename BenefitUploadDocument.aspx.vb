...
Public Class BenefitUploadDocument
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        query = "select * from  [" & My.MySettings.Default.COMPANYNAME.Trim & "$Benefit Req_ Required Document] where [Benefit Request Type]= '" & reqtypel & "' and [Sector] in (0," & sector & ")  and [Category] = '" & units & "'"
        ...
        For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
            ...
            If Not (docObj.Contains(flname)) Then
                Dim existds As New DataSet
                ' newly added for partial on 2nd of may, 2017'

                query = "select * from [" & My.MySettings.Default.COMPANYNAME.Trim & "$Benefit Request Files] where [Document Name] = '" & flname & "' and [Code] = '" & Nos & "'"
                existds = returndataset(query)
                If existds.Tables(0).Rows.Count = 0 Then
                    docObj.Add(flname)
                    OptionalArray.Add("1")
                Else
                    rForm.Visible = False
            ...
            End If
        Next
        If docObj.Count = 0 Then
            rForm.Visible = False
            writefailure("All required document(s) already attached")
            btnsave.Visible = False
            Exit Sub
        End If
        If IsPostBack Then
